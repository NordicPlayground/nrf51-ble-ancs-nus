...
static ble_ancs_c_t *        m_ancs_c_obj;                                                 /**< Pointer to the instantiated object. */
...
static void ancs_pstorage_callback(pstorage_handle_t * handle,
                                   uint8_t             op_code,
                                   uint32_t            reason,
                                   uint8_t           * p_data,
                                   uint32_t            param_len)
{
    ...
    {
        m_ancs_c_obj->error_handler(reason);
    }
}
...
uint32_t ble_ans_c_service_load(const ble_ancs_c_t * p_ans)
{
    uint32_t err_code;
    uint32_t i;
    ...
    if (err_code != NRF_SUCCESS)
    {
        ...
        for (i = 0; i < BLE_ANCS_MAX_DISCOVERED_CENTRALS; ++i)
        {
